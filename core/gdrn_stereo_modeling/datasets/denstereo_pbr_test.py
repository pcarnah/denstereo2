...
class DENSTEREO_PBR_Dataset:
    def __init__(self, data_cfg):
        ...
        self.dataset_root_l = data_cfg.get(
            "dataset_root",
            osp.join(DATASETS_ROOT, "train_pbr_left")
        )
        self.dataset_root_r = data_cfg.get(
            "dataset_root_right",
            osp.join(DATASETS_ROOT, "train_pbr_right")
        )
        ...
        self.cat_ids = [cat_id for cat_id, obj_name in denstereo.id2obj.items() if obj_name in self.objs]
        ...
        cam_path = osp.join(DATASETS_ROOT, 'camera.json')
    ...
    def __call__(self):
        ...
        if osp.exists(cache_path) and self.use_cache:
            ...
            dataset_dict = mmcv.load(cache_path)
            dataset_dict_filt = [d for d in dataset_dict if d is not None]
            return dataset_dict_filt
        ...
        def load_scene(scene):
            ...
            if (not osp.exists(osp.join(scene_root_l, "scene_gt.json")) or
            not osp.exists(osp.join(scene_root_r, "scene_gt.json")) or
            not osp.exists(osp.join(scene_root_l, "scene_gt_info.json")) or
            not osp.exists(osp.join(scene_root_r, "scene_gt_info.json")) or
            not osp.exists(osp.join(scene_root_l, "scene_camera.json")) or
            not osp.exists(osp.join(scene_root_r, "scene_camera.json"))): return []
            ...
            annotations_path_l = osp.join(scene_root_l, "annotations")
            annotations_path_r = osp.join(scene_root_r, "annotations")

            mmcv.mkdir_or_exist(annotations_path_l)
            mmcv.mkdir_or_exist(annotations_path_r)

            # for str_im_id in tqdm(gt_dict_l, position=scene_id):
            # for str_im_id in gt_dict_l:
            def load_record(str_im_id):
                ...
                mmcv.dump(insts, ann_path_l, protocol=4)

                # scene_dataset_dicts.append(record)
                return [record]

            with ThreadPoolExecutor(12) as executor:
                scene_dataset_dicts = list(chain.from_iterable(executor.map(load_record, gt_dict_l.keys())))
                executor.shutdown()

            end = timer()
            print(f"{scene_id:06d} done in {(end - start) / 60.0:.2f} minutes")
            return scene_dataset_dicts
        with Pool(6) as pool:
            dataset_dicts = list(chain.from_iterable(pool.map(load_scene, self.scenes)))

        # with ThreadPoolExecutor() as executor:
        #     dataset_dicts = list(chain.from_iterable(executor.map(load_scene, self.scenes)))
        #     executor.shutdown()
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            model = inout.load_ply(
                osp.join(self.models_root, f"obj_{denstereo.obj2id[obj_name]:06d}.ply"), vertex_scale=self.scale_to_meter
            )
...
def get_denstereo_metadata(obj_names, ref_key):
    ...
    loaded_models_info = denstereo.get_models_info()

    for i, obj_name in enumerate(obj_names):
        obj_id = denstereo.obj2id[obj_name]
...
SPLITS_DENSTEREO_PBR = dict(
    denstereo_train_pbr =dict(
        ...
        objs=denstereo.objects,  # selected objects
        dataset_root=osp.join(DATASETS_ROOT, "train_pbr_left"),
        dataset_root_right=osp.join(DATASETS_ROOT, "train_pbr_right"),
        models_root=osp.join(DATASETS_ROOT, "models"),
        xyz_root=osp.join(DATASETS_ROOT, "train_pbr_left/xyz_crop_hd"),
        xyz_root_right=osp.join(DATASETS_ROOT, "train_pbr_right/xyz_crop_hd"),
        occ_root=osp.join(DATASETS_ROOT, "train_pbr_left/Q0"),
        occ_root_right=osp.join(DATASETS_ROOT, "train_pbr_right/Q0"),
    ...
    denstereo_train_pbr_right =dict(
        ...
        objs=denstereo.objects,  # selected objects
        dataset_root=osp.join(DATASETS_ROOT, "train_pbr_right"),
        models_root=osp.join(DATASETS_ROOT, "models"),
        xyz_root=osp.join(DATASETS_ROOT, "train_pbr_right/xyz_crop_hd"),
        occ_root=osp.join(DATASETS_ROOT, "train_pbr_right/Q0"),
        scale_to_meter=0.001,
        with_masks=True,  # (load masks but may not use it)
        with_depth=True,  # (load depth path here, but may not use it)
        height=480,
    ...
    denstereo_test_pbr =dict(
        ...
        num_to_load=-1,
        filter_invalid=True,
        scenes=denstereo.test_pbr_scenes,
        ref_key="denstereo",
    ),
    ...
    denstereo_debug_train_pbr =dict(
        ...
        objs=[denstereo.objects[0]],  # selected objects
        dataset_root=osp.join(DATASETS_ROOT, "train_pbr_left"),
        dataset_root_right=osp.join(DATASETS_ROOT, "train_pbr_right"),
        models_root=osp.join(DATASETS_ROOT, "models"),
        xyz_root=osp.join(DATASETS_ROOT, "train_pbr_left/xyz_crop_hd"),
        ...
        num_to_load=-1,
        filter_invalid=True,
        scenes=denstereo.debug_pbr_scenes,
        max_im = 100,
        ref_key="denstereo",
    ),
    denstereo_debug_test_pbr =dict(
        ...
        num_to_load=-1,
        filter_invalid=True,
        scenes=denstereo.debug_test_pbr_scenes,
        max_im = 100,
        ref_key="denstereo",
    ),
    denstereo_debug_train_pbr_left =dict(
        name="denstereo_debug_train_pbr_left",
        # objs=[denstereo.objects[1]],  # selected objects
        objs=denstereo.objects,  # selected objects
        dataset_root=osp.join(DATASETS_ROOT, "train_pbr_left"),
        models_root=osp.join(DATASETS_ROOT, "models"),
        xyz_root=osp.join(DATASETS_ROOT, "train_pbr_left/xyz_crop_hd"),
...
for split in ['train_pbr_left']: # TODO add train _right 'train_pbr_right']:
    for scene in denstereo.debug_pbr_scenes:
        scene_root = osp.join(DATASETS_ROOT, "{:s}/{:06d}".format(
                split,
                scene
            )
        )
        ...
        for im_id in gt_dict.keys():
            ...
            for obj_id in obj_ids:
                ...
                if name not in SPLITS_DENSTEREO_PBR:
                    ...
                    SPLITS_DENSTEREO_PBR[name] = dict(
                        name=name,
                        dataset_root=osp.join(DATASETS_ROOT, "{:s}".format(split)),
                        models_root=osp.join(DATASETS_ROOT, "models"),
                        objs=denstereo.objects,  # only this obj
                        image_prefixes=[
                            osp.join(DATASETS_ROOT, "{:s}/{:06d}").format(split, scene)
                        ],
                        xyz_prefixes=[
                            osp.join(DATASETS_ROOT, "{:s}/xyz_crop_hd/{:06d}".format(split, scene))
                        ],
                        scale_to_meter=0.001,
                        with_masks=True,  # (load masks but may not use it)
                        with_depth=True,  # (load depth path here, but may not use it)
        ...
        for obj_id in obj_ids:
            ...
            if name not in SPLITS_DENSTEREO_PBR:
                ...
                SPLITS_DENSTEREO_PBR[name] = dict(
                    ...
                    objs=denstereo.objects,  # only this obj
                    image_prefixes=[
                        osp.join(DATASETS_ROOT, "train_pbr_left/{:06d}").format(scene)
                    ],
                    image_prefixes_right=[
                        osp.join(DATASETS_ROOT, "train_pbr_right/{:06d}").format(scene)
                    ],
                    xyz_prefixes=[
                        osp.join(DATASETS_ROOT, "train_pbr_left/xyz_crop_hd/{:06d}".format(scene))
                    ],
                    xyz_prefixes_right=[
                        osp.join(DATASETS_ROOT, "train_pbr_right/xyz_crop_hd/{:06d}".format(scene))
                    ],
                    scale_to_meter=0.001,
                    with_masks=True,  # (load masks but may not use it)
                    with_depth=True,  # (load depth path here, but may not use it)
...
def test_vis():
    ...
    dicts2 = get_detection_dataset_dicts(dset_name, filter_empty=True)
    ...
    for d in dicts:
        img = read_image_mmcv(d["file_name_l"], format="BGR")
        depth = mmcv.imread(d["depth_file_l"], "unchanged") / 10000.0

        imH, imW = img.shape[:2]
        annos = d["annotations"]
        masks = [cocosegm2mask(anno["segmentation_l"], imH, imW) for anno in annos]
        bboxes = [anno["bbox_l"] for anno in annos]
        ...
        quats = [anno["quat_l"] for anno in annos]
        transes = [anno["trans_l"] for anno in annos]
        ...
        for _i in range(len(annos)):
            ...
            xyz_path = annos[_i]["xyz_path_l"]
            with np.load(xyz_path) as xyz_info:
